...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
        ...
        if (nHeight % GetBudgetPaymentCycleBlocks() < 1) {           
            return true;
        } else {
        ...
        if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && ( nHeight % GetBudgetPaymentCycleBlocks() < 1) ) {
            // foundation superblock
            // check to make sure that the value is exactly as expected.
            CAmount nFoundationValue = ((GetBudgetPaymentCycleBlocks() * 4) + 1) * COIN;

            if(nMinted <= nFoundationValue) {
                return true;
            } else {
                LogPrintf("Foundation() : Warning - Foundation Value Check Failed\n");
                LogPrintf("Foundation() : nMinted: %s\n", FormatMoney(nMinted).c_str() );
                LogPrintf("Foundation() : nFoundationValue: %s\n", FormatMoney(nFoundationValue).c_str() );
                return false;
            }
        }
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    //if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS)) {
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && ( (pindexPrev->nHeight + 1) % GetBudgetPaymentCycleBlocks() == 0) ) {
...
std::string GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    if ( pindexPrev && IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && ( (pindexPrev->nHeight + 1) % GetBudgetPaymentCycleBlocks() == 0) ) {    
