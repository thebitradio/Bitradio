...
#if defined(NDEBUG)
#error "BitRadio cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    if (nHeight == 0) {
        return 1 * COIN;
    } else if (nHeight > 0 && nHeight <= 1) {
        return 12500000 * COIN; // premine of previous chain
    } else {
        return 1 * COIN;
    }
}
