...
int GetBudgetPaymentCycleBlocks()
{
    ...
    return 10;  // often, good for testing
}

bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    // budgets are turned off for BitRadio.
    return false;
...
void CBudgetManager::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake)
{
    ...
    CAmount broPaymentAmt;
    broPaymentAmt = GetBudgetPaymentCycleBlocks() * 4 * COIN;

    if( Params().NetworkID() == CBaseChainParams::MAIN ) {
        broPayee = GetScriptForDestination( CBitcoinAddress("BkSBPg6xtTJSuUBdhJWdEPeRMreS6TBVTq").Get() );
    } else {
        broPayee = GetScriptForDestination( CBitcoinAddress("xxpmfWQP4rrypxndAsp894h1qKqs92DKXZ").Get() );
    }    
    ...
    if (fProofOfStake) {
        // for BitRadio, there won't be any proposals.
        // nHighestCount shouldn't ever be above 0.
        if (nHighestCount > 0) {
            ...
            // LogPrint("mnbudget","CBudgetManager::FillBlockPayee - Budget payment to %s for %lld, nHighestCount = %d\n", CBitcoinAddress(address1).Get(), nAmount, nHighestCount);
        }
        else {
            // This is where the foundation magic happens
            // -------- during the PoS phase ------------
            unsigned int i = txNew.vout.size();
            txNew.vout.resize(i + 1);
            txNew.vout[i].scriptPubKey = broPayee;
            txNew.vout[i].nValue = broPaymentAmt;

            LogPrint("mnbudget","CBudgetManager::FillBlockPayee - Foundation Payment of %lld, nHighestCount = %d\n", broPaymentAmt, nHighestCount);
        }
    } else {
        // miners will not get the full amount on these blocks
        // for BitRadio, SuperBlocks get paid directly to the foundation
        // wallet for direct streaming payouts.
        ...
        if (nHighestCount > 0) {
            ...
            // LogPrint("mnbudget","CBudgetManager::FillBlockPayee - Budget payment to %s for %lld\n", CBitcoinAddress(address1).Get(), nAmount);
        } else {
            // since there are no valid budgets in BitRadio, this will always fire 
            // on mined superblocks (mainly for testnet)
            // -----------------------------------------
            txNew.vout.resize(2);
            txNew.vout[1].scriptPubKey = broPayee;
            txNew.vout[1].nValue = broPaymentAmt;
